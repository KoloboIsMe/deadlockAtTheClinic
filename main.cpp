...
void takeCareOfPatient()
{
    ...
    if(patient.try_lock() == false) { // Si il n'y a personne dans la file d'attente
        ...
        medicWakeUp.lock();    // Le médecin s'endort
        medicWakeUp.lock();    // Attend qu'on le réveille (il prend la ressource et attend que quelqu'un d'autre la libère)
        medicWakeUp.unlock();  // Il se réveille
